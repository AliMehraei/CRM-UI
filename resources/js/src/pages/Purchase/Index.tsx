...
const List = () => {
    ...
    useEffect(() => {
        if (!isLoading && !hasPermission('filter-purchase-order') && !hasPermission('read-purchase-order')) {
            setLoading(true);
    ...
    const fetchDataFilterOption = async () => {
        setLoading(true);
        try {
            const res = await api_instance.filterOptionPurchaseOrder();
            // Transform the data
    ...
    const applyFilters = () => {
        ...
        scrollToTop();
        fetchDatapurchaseOrder(page, pageSize, filters, sortStatus);
        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSinglePurchaseOrder(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the purchaseOrder: ' + result.message, 'error');
                                console.error('Error deleting the purchaseOrder', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDatapurchaseOrder = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        setLoading(true);
        ...
        try {
            api_instance.fetchDataPurchaseOrder({
                page: page,
                pageSize: pageSize,
                sortField: sortField,
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching purchaseOrder data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching purchaseOrder data.', 'error');
            console.error('Error fetching data:', error);
    ...
    const handleSortChange = (sortStatus) => {
        ...
        setPage(1);
        fetchDatapurchaseOrder(page, pageSize, filters, { columnAccessor, direction });
    };
            ...
            <div className="panel px-0 border-white-light dark:border-[#1b2e4b]" >
                <div className="purchaseOrder-table">
                    <div className="mb-4.5 px-5 flex md:items-center md:flex-row flex-col gap-5">
                        <div className="flex items-center gap-2">
                            ...
                            {hasPermission('create-purchase-order') && (
                                <Link to="/purchase/add" className="btn btn-primary gap-2">
                                    Add New
                                </Link>
                            )}
                    ...
                    <div className="grid grid-cols-5 gap-6 mb-6">
                        {hasPermission('filter-purchase-order') && (
                                            ...
                                            },
                                            {
                                                ...
                                                render: ({ id }) => (
                                                    <div className="flex gap-4 items-center w-max mx-auto">
                                                        {hasPermission('delete-purchase-order') && (
                                                            <NavLink to={`/purchase/edit/${id}`} className="flex hover:text-info">
                                                        ...
                                                        {/* <NavLink to="/purchase/preview" className="flex hover:text-primary">
