...
import {displayImage, formatDate, getStatusLabel, StatusOption} from '../../components/Functions/CommonFunctions';
...
const Preview = () => {
    ...
    const purchaseOrderHeaderSection = {
        'leftObjects': [
            ...
            {label: "Availability No", value: formState.availability?.availability_name},
    ...
    const PurchaseOrderInformationSection =
        {
            ...
            'rightObjects': [
                ...
                {label: "Confirmed DD", value: formState.confirmed_delivery_date},
                {label: "Carrier", value: `${getStatusLabel(formState.carrier, CarrierOption)}`},
                {label: "AWB", value: formState.awb},
                {
                    label: "Confirmation Vendor", value: (<AttachmentDownloadButton
    ...
    const POLineSection = {
        'leftObjects': [
            {label: "Requested Delivery Date", value: formState.requested_delivery_date},
        ...
        'rightObjects': [
            {label: "Confirmed Delivery Date", value: formState.confirmed_delivery_date},
    ...
    const LinkedSO = {
        'leftObjects': [
            {label: "Account", value: `${formState.sales_order?.account?.account_name}`},
            ...
            {label: "SO Status", value: `${getStatusLabel(formState.sales_order?.status, StatusOption)}`},
        ],
        'rightObjects': [
            {label: "Requested D.D.", value: `${formState.sales_order?.requested_delivery_date}`},
            {label: "Confirmed D.D.", value: `${formState.sales_order?.confirmed_delivery_date}`},
    ...
    return (
        (!hasPermission(`read-purchase-order`) || loading) ? (
            <LoadingSasCrm/>
        ) : (
            <div>
                ...
                <div className="panel">
                    <div className="flex justify-between flex-wrap gap-4 px-4">
                        <div className="text-2xl font-semibold uppercase">Purchase Order</div>
                        <div className="shrink-0">
                            <img src={displayImage(formState.image_data)} alt="Purchase Order image"
