...
const List = () => {
    ...
    useEffect(() => {
        if (!isLoading && !hasPermission('filter-excess') && !hasPermission('read-excess')) {
            setLoading(true);
    ...
    const fetchDataFilterOption = async () => {
        setLoading(true);
        try {
            const res = await api_instance.filterOptionExcess();
            // Transform the data
    ...
    const applyFilters = () => {
        ...
        scrollToTop();
        fetchDataExcess(page, pageSize, filters, sortStatus);
        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSingleExcess(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the excess: ' + result.message, 'error');
                                console.error('Error deleting the excess', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDataExcess = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        setLoading(true);
        ...
        try {
            api_instance.fetchDataExcess({
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching excess data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching excess data.', 'error');
            console.error('Error fetching data:', error);
    ...
    const handleSortChange = (sortStatus) => {
        ...
        setPage(1);
        fetchDataExcess(page, pageSize, filters, { columnAccessor, direction });
    };
                                            ...
                                            },
                                            {
                                                ...
                                                render: ({ id }) => (
                                                    <div className="flex gap-4 items-center w-max mx-auto">
                                                        {hasPermission('delete-excess') && (
                                                            <NavLink to={`/excess/edit/${id}`} className="flex hover:text-info">
                                                        ...
                                                        {/* <NavLink to="/excess/preview" className="flex hover:text-primary">
