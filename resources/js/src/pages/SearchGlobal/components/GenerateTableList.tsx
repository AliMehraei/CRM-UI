...
const GenerateTableList = ({ permissionName, tableColumns, frontRoute , filterParam, title}: any) => {
    ...
    const fetchModelData = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            ...
            await api_instance[frontRoute]({
                ...
                res.data.forEach((modelData: any) => {
                    const modelName = Object.keys(modelData)[0]; // Get the model name
                    const modelArray = modelData[modelName]; // Get the array of data for the model
    
                    // Process the data for the model as needed
                    console.log(`Model: ${modelName}`);
                    console.log('Data:', modelArray);
    
                    // You can now use modelArray as needed, for example, set it in the state
                    setItems(modelArray);
                });
