...
import LoadingSasCrm from '../../components/LoadingSasCrm';const List = () => {
    ...
    useEffect(() => {
        if (!isLoading && !hasPermission('filter-rfq') && !hasPermission('read-rfq')) {
            setLoading(true);
        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSingleRfq(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the rfq: ' + result.message, 'error');
                                console.error('Error deleting the rfq', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDataRfq = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            api_instance.fetchDataRfq({
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching rfq data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching rfq data.', 'error');
            console.error('Error fetching data:', error);
        ...
        <div className="panel px-0 border-white-light dark:border-[#1b2e4b]" >
            <div className="rfq-table">
                <div className="mb-4.5 px-5 flex md:items-center md:flex-row flex-col gap-5">
                    ...
                    <Link to="/rfq/add" className="btn btn-primary gap-2">
                        Add New
                    </Link>
                    )}
                                    ...
                                    columns={[
                                        ...
                                        },
                                        {
                                            ...
                                            render: ({ id }) => (
                                                ...
                                                {hasPermission('delete-rfq') && (
                                                <NavLink to={`/rfq/edit/${id}`} className="flex hover:text-info">
                                                ...
                                                {/* <NavLink to="/rfq/preview" className="flex hover:text-primary">
