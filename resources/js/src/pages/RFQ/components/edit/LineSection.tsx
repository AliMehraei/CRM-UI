...
export const LineSection = () => {
    ...
    const [availabilitySuggestionsData, setAvailabilitySuggestionsData] =
        useState<AvailabilitySuggestionsData>({
            web: [],
            nonWeb: [],
        });
    const [excessSuggestionsData, setExcessSuggestionsData] =
        useState<ExcessSuggestionsData>({ matched: [] });
    ...
    const ProductSuggestionsTable: React.FC<{
        suggestions: AvailabilitySuggestionsData;
        loading: boolean;
    }> = ({ suggestions, loading }) => (
        <>
                        ...
                        {suggestions.web.map((suggestion, index) => (
                            ...
                            >
                                <td className="py-2 px-4 border-b">
                                    {suggestion.availability_name}
                                </td>
                                <td className="py-2 px-4 border-b">
                                    {suggestion.availability_source}
                                </td>
                                <td className="py-2 px-4 border-b">
                                    {suggestion.cost} {suggestion.currency}
                                </td>
                                <td className="py-2 px-4 border-b">
    ...
    const ExcessSuggestionsTable: React.FC<{
        suggestions: ExcessSuggestionsData;
        loading: boolean;
    }> = ({ suggestions, loading }) => (
        <>
                ...
                {loading ? (
                    ...
                    <tbody>
                        {suggestions.matched.map((suggestion, index) => (
                            ...
                            >
                                <td className="py-2 px-4 border-b">
                                    {suggestion.excess_name}
                                </td>
                                <td className="py-2 px-4 border-b">
                                    {suggestion.excess_source}
                                </td>
                                <td className="py-2 px-4 border-b">
                                    {suggestion.excess_no}
                                </td>
                                <td className="py-2 px-4 border-b">
                                    {suggestion.cost} {suggestion.currency}
                                </td>
                                <td className="py-2 px-4 border-b">
                                    {suggestion.excess_type}
                                </td>
                                <td className="py-2 px-4 border-b">
                                    {suggestion.quantity}
                                </td>
            ...
            "Product Name": (
                <AsyncSelect
                    ...
                    defaultValue={{
                        ...
                        label: (
                            <div
                                key={formState.product?.id}
                                className="flex items-center"
                            >
                                {formState.product ? (
                                    <>
                                        <div className="text-sm font-bold">
                                            {formState.product?.product_name}
                                        </div>
                                        <button
                                            className="btn text-xs btn-sm ml-auto"
                                            onClick={() =>
                                                handleCopySelect(
                                                    `${formState.product?.product_name}`
                                                )
        ...
        "": {
            "Alternative Products": (
                <AsyncSelect
                    ...
                    isMulti={true}
                    defaultValue={
                        formState.rfq_product_alternatives
                            ? formState.rfq_product_alternatives.map(
                                  (data: any) => ({
                                      value: data.id,
                                      label: (
                                          <div
                                ...
                                {formState.availability ? (
                                    <>
                                        ...
                                        <button
                                            ...
                                            onClick={() =>
                                                handleCopySelect(
                                                    `${formState.availability?.availability_name}`
                                                )
                                            }
                                        >
                                            Copy & Select
            ...
            Excess: (
                <AsyncSelect
                    ...
                    defaultValue={{
                        ...
                        label: (
                            <div
                                key={formState.excess?.id}
                                className="flex items-center"
                            >
                                {formState.excess ? (
                                    <>
                                        <div className="text-sm font-bold">
                                            {formState.excess?.excess_name}
                                        </div>
                                        <button
                                            className="btn text-xs btn-sm ml-auto"
                                            onClick={() =>
    ...
    const fetchAvailabilitySuggestions = async (productId: string) => {
        try {
            ...
            if (response.status === 200) {
                ...
                setAvailabilitySuggestionsData({
                    web: webSuggestions,
