...
const List = () => {
    ...
    useEffect(() => {
        if (!isLoading && !hasPermission('filter-invoice') && !hasPermission('read-invoice')) {
            setLoading(true);
    ...
    const applyFilters = () => {
        ...
        scrollToTop();
        fetchDataInvoice(page, pageSize, filters,sortStatus);
    ...
    const fetchDataInvoice = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        setLoading(true);
    ...
    const handleSortChange = (sortStatus) => {
        ...
        setPage(1);
        fetchDataInvoice(page, pageSize, filters, { columnAccessor, direction });
    };
