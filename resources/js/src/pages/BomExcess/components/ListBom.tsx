...

        {
            ...
            render: ({ id, status} : any) => (
                <>
                    ...
                    <a href={`/`+btnRoute+ `/confirmation/` + contactId + `/` +id}
                        ...
                        <svg className="w-5 h-5 ml-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.348a1.125 1.125 0 010 1.971l-11.54 6.347a1.125 1.125 0 01-1.667-.985V5.653z" />
                        </svg>
                    </a>
                    )}

                    {['pre_processing', 'pre_processed_with_failures','pre_processed_successfully'].includes(status) && (
                    <a href={`/`+btnRoute+ `/process/` + contactId + `/` +id}
                    className="bg-white p-2 rounded-md text-primary-500 border border-primary-500 hover:text-white hover:bg-sky-600 default-transition pulse-primary flex items-center justify-center">
                    ...
                    <a href={`/`+btnRoute+ `/complete/` + contactId + `/` +id}
