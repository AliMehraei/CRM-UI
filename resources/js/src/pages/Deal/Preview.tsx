...
const Preview = () => {
    ...
    const headerDataToDisplay = [
        ...
        { label: "Account Owner", value: `${formState.account?.first_name ?? ''} ${formState.account?.last_name ?? ''}` },
        { label: "Deal Owner", value: `${formState.owner?.first_name ?? ''} ${formState.owner?.last_name ?? ''}` },
                ...
                <div className="panel">
                    <div className="flex justify-between flex-wrap gap-4 px-4">
                        ...
                        <div className="shrink-0">
                            <img src={displayImage(formState.image_data)} alt="Deal image" className="w-20 ltr:ml-auto rtl:mr-auto" />
                        </div>
                    </div>
                    ...
                    <InformationSectionPreview
                        ...
                        leftObjects={[
                            ...
                            {
                                label: "Contact Name",
                                value: `${formState.contact?.first_name ?? ''} ${formState.contact?.last_name ?? ''}`
                            },                            
                            {
                                label: "Lead",
                                value: `${formState.lead?.first_name ?? ''} ${formState.lead?.last_name ?? ''}`
                            },   
                            {
                                label: "Invoice",
                                value: `${formState.invoice?.subject ?? ''}`
                            },                          
                            { label: "RFQ", value:`${formState.invoice?.rfq_name ?? ''}` },
                            { label: "Quote", value: `${formState.quote?.subject ?? ''}` },
                            { label: "Sales Order", value:`${formState.invoice?.sales_order ?? ''}` } ,
                        ]}
                        rightObjects={[
                            { label: "Amount", value: formState.amount },
                            { label: "Closing Date", value: formState.closing_date },
