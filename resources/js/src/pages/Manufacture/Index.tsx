...
const List = () => {
    ...
    const fetchDataFilterOption = async () => {
        setLoading(true);
        try {
            const res = await api_instance.filterOptionManufacturer();
            // Transform the data
    ...
    const applyFilters = () => {
        ...
        scrollToTop();
        fetchDatamanufacture(page, pageSize, filters);
        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSingleManufacturer(rowId).then((res) => {
    ...
    const fetchDatamanufacture = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            api_instance.fetchDataManufacturer({
                page: page,
                pageSize: pageSize,
                sortField: sortField,
    ...
    useEffect(() => {
        fetchDatamanufacture(page, pageSize);
