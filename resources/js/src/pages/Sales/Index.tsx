...
const List = () => {
    ...
    useEffect(() => {
        if (!isLoading && !hasPermission('filter-sales-order') && !hasPermission('read-sales-order')) {
            setLoading(true);
    ...
    const fetchDataFilterOption = async () => {
        setLoading(true);
        try {
            const res = await api_instance.filterOptionSalesOrder();
            // Transform the data
        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSingleSalesOrder(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the salesOrder: ' + result.message, 'error');
                                console.error('Error deleting the salesOrder', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDatasalesOrder = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            api_instance.fetchDataSalesOrder({
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching salesOrder data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching salesOrder data.', 'error');
            console.error('Error fetching data:', error);
    ...
    const handleSortChange = (sortStatus) => {
        ...
        setPage(1);
        fetchDatasalesOrder(page, pageSize, filters, { columnAccessor, direction });
    };
    ...
    return (
        ...
        <div className="panel px-0 border-white-light dark:border-[#1b2e4b]" >
            <div className="salesOrder-table">
                <div className="mb-4.5 px-5 flex md:items-center md:flex-row flex-col gap-5">
                    ...
                    <Link to="/sales/add" className="btn btn-primary gap-2">
                        Add New
                    </Link>
                    )}
                ...
                {hasPermission('filter-sales-order') && (
                                        ...
                                        },
                                        {
                                            ...
                                            render: ({ id }) => (
                                                <div className="flex gap-4 items-center w-max mx-auto">
                                                    {hasPermission('delete-sales-order') && (
                                                <NavLink to={`/sales/edit/${id}`} className="flex hover:text-info">
                                                ...
                                                {/* <NavLink to="/sales/preview" className="flex hover:text-primary">
