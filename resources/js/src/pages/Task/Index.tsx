...
const List = () => {
    ...
    const fetchDataFilterOption = async () => {
        setLoading(true);
        try {
            const res = await api_instance.filterOptionTask();
            // Transform the data
        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSingleTask(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the Task: ' + result.message, 'error');
                                console.error('Error deleting the Task', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDataTask = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            api_instance.fetchDataTask({
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching task data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching task data.', 'error');
            console.error('Error fetching data:', error);
            ...
            <div className="task-table">
                <div className="mb-4.5 px-5 flex md:items-center md:flex-row flex-col gap-5">
                    <div className="flex items-center gap-2">
                        ...
                        <Link to="/task/add" className="btn btn-primary gap-2">
                            Add New
                        </Link>
                        )}
                    </div>
                                        ...
                                        },
                                        {
                                            ...
                                            render: ({ id }) => (
                                                <div className="flex gap-4 items-center w-max mx-auto">
                                                    {hasPermission('update-task') && (
                                                    <NavLink to={`/task/edit/${id}`} className="flex hover:text-info">
                                                    ...
                                                    {/* <NavLink to="/task/preview" className="flex hover:text-primary">
