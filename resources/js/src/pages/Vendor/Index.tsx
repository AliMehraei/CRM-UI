...
const List = () => {
    ...
    useEffect(() => {
        if (!isLoading && !hasPermission('filter-vendor') && !hasPermission('read-vendor')) {
            setLoading(true);
    ...
    const applyFilters = () => {
        ...
        scrollToTop();
        fetchDatavendor(page, pageSize, filters, sortStatus);
                    ...
                    try {
                        ...
                        api_instance.deleteSingleVendor(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the vendor: ' + result.message, 'error');
                                console.error('Error deleting the vendor', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDatavendor = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            api_instance.fetchDataVendor({
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching vendor data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching vendor data.', 'error');
            console.error('Error fetching data:', error);
    ...
    const handleSortChange = (sortStatus) => {
        ...
        setPage(1);
        fetchDatavendor(page, pageSize, filters, { columnAccessor, direction });
    };
        ...
        <div className="panel px-0 border-white-light dark:border-[#1b2e4b]" >
            <div className="vendor-table">
                <div className="mb-4.5 px-5 flex md:items-center md:flex-row flex-col gap-5">
                    ...
                    <Link to="/vendor/add" className="btn btn-primary gap-2">
                        Add New
                    </Link>
                    )}
                    </div>
                                        ...
                                        },
                                        {
                                            ...
                                            render: ({ id }) => (
                                                <div className="flex gap-4 items-center w-max mx-auto">
                                                    ...
                                                    {hasPermission('delete-vendor') && (
                                                <NavLink to={`/vendor/edit/${id}`} className="flex hover:text-info">
                                                ...
                                                {/* <NavLink to="/vendor/preview" className="flex hover:text-primary">
