...
const List = () => {
    ...
    useEffect(() => {
        if (!isLoading && !hasPermission('filter-account') && !hasPermission('read-account')) {
            setLoading(true);
    ...
    const fetchDataFilterOption = async () => {
        setLoading(true);
        try {
            const res = await api_instance.filterOptionAccount();
            // Transform the data
        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSingleAccount(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the account: ' + result.message, 'error');
                                console.error('Error deleting the account', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDataAccount = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            api_instance.fetchDataAccount({
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching account data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching account data.', 'error');
            console.error('Error fetching data:', error);
    ...
    const handleSortChange = (sortStatus) => {
        ...
        setPage(1);
        fetchDataAccount(page, pageSize, filters, { columnAccessor, direction });
    };
