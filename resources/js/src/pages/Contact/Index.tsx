        ...
        Swal.fire({
            ...
            if (result.value) {
                const deleteSingleRow = async (rowId: number) => {
                    try {
                        setLoading(true);
                        api_instance.deleteSingleContact(rowId).then((res) => {
                            ...
                            if (result.status) {
                                ...
                                showMessage('Error deleting the contact: ' + result.message, 'error');
                                console.error('Error deleting the contact', result.message);
                            }
                        });
                        setLoading(false);
                    } catch (error) {
    ...
    const fetchDataContact = async (page = 1, pageSize = PAGE_SIZES[0], filters = [], sortStatus = {}) => {
        ...
        try {
            api_instance.fetchDataContact({
                ...
                console.error('Error fetching data:', error);
                setLoading(false);
                showMessage('Error fetching contact data.', 'error');
            });
        } catch (error) {
            showMessage('Error fetching contact data.', 'error');
            console.error('Error fetching data:', error);
